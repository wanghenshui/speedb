...
TEST_F(DBTest, CustomComparator) {
  ...
  class NumberComparator : public Comparator {
   ...
   private:
    static int ToNumber(const Slice& x) {
      return NumberConverter::ToNumber(x);
    }
  };
   ...
   public:
    ...
    void CreateFilter(const Slice* keys, int n,
      ...
      for (const auto& num : converted) {
        sliced.push_back(Slice(reinterpret_cast<const char*>(&num), sizeof(num)));
      }
    ...
    FilterBitsBuilder* GetBuilderWithContext(
      ...
      return new CustomFilterBitsBuilder(wrapped_->GetBuilderWithContext(context));
    }
  ...
  do {
    ...
    BlockBasedTableOptions* table_options = new_options.table_factory->GetOptions<BlockBasedTableOptions>();
    if (table_options) {
