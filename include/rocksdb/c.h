...
typedef enum {
  ...
  kIOFenced = 14,
  kMaxSubCode
} rocksdb_status_sub_code;

typedef struct {
  rocksdb_status_code code;
  rocksdb_status_sub_code subcode;
} rocksdb_status;
...
} rocksdb_compaction_job_stats;

typedef struct {
  // the status indicating whether the compaction was successful or not.
  rocksdb_status status;
...
} rocksdb_flush_table_properties;
...
typedef struct {
  ...
  rocksdb_flush_table_properties table_properties;
} rocksdb_flush_job_info;

typedef void (*flush_started_cb)(void* context, rocksdb_flush_job_info*);
typedef void (*flush_completed_cb)(void* context, rocksdb_flush_job_info*);
typedef void (*compaction_started_cb)(void* context,
                                      rocksdb_compaction_job_info*);
typedef void (*compaction_completed_cb)(void* context,
                                        rocksdb_compaction_job_info*);
...
extern ROCKSDB_LIBRARY_API void rocksdb_options_add_event_listener_cb(
    rocksdb_options_t* opt, void* context, flush_started_cb flush_start,
    flush_completed_cb flush_compl, compaction_started_cb comp_start,
...
extern ROCKSDB_LIBRARY_API void rocksdb_options_set_allow_os_buffer(
    rocksdb_options_t* options, int val);
