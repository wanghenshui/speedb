...
#include "memtable/hash_spd_rep.h"
...
struct BucketHeader {
  ...
  bool Contains(const MemTableRep::KeyComparator& comparator,
    ...
    for (auto k = items_; k != nullptr; k = k->next) {
      const int cmp_res = comparator(k->data, check_key);
      if (cmp_res == 0) {
        return true;
      }
      if (cmp_res > 0) {
  ...
  bool Add(SpdbKeyHandle* val, const MemTableRep::KeyComparator& comparator) {
    ...
    for (; *loc != nullptr; loc = &(*loc)->next) {
      const int cmp_res = comparator((*loc)->data, val->data);
      if (cmp_res == 0) {
        return false;
      }
      if (cmp_res > 0) {
...
struct SpdbHashTable {
  ...
  bool Add(SpdbKeyHandle* val, const MemTableRep::KeyComparator& comparator) {
    auto mutex_and_bucket = GetMutexAndBucket(val->data, comparator);
  ...
  void Get(const LookupKey& k, const MemTableRep::KeyComparator& comparator,
    ...
    for (; iter != nullptr; iter = iter->next) {
      if (comparator(iter->data, k.internal_key()) >= 0) {
        break;
      }
    }

    for (; iter != nullptr; iter = iter->next) {
      if (!callback_func(callback_args, iter->data)) {
        break;
      }
    }
  }
...
HashLocklessRep::~HashLocklessRep() { MarkReadOnly(); }

KeyHandle HashLocklessRep::Allocate(const size_t len, char** buf) {
  constexpr size_t kInlineDataSize =
      sizeof(SpdbKeyHandle) - offsetof(SpdbKeyHandle, data);
...
bool HashLocklessRep::InsertKey(KeyHandle handle) {
  ...
  spdb_vectors_cont_->Insert(spdb_handle->data);
  return true;
}
...
void HashLocklessRep::MarkReadOnly() { spdb_vectors_cont_->MarkReadOnly(); }
...
MemTableRep* HashLocklessRepFactory::CreateMemTableRep(
    const MemTableRep::KeyComparator& compare, Allocator* allocator,
    const SliceTransform* /*transform*/, Logger* /* logger */) {
  return new HashLocklessRep(compare, allocator, bucket_count_, 10000);
}

MemTableRepFactory* NewHashLocklessRepFactory(size_t bucket_count) {
  return new HashLocklessRepFactory(bucket_count);
}
