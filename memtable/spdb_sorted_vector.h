...
enum SeekOption {
  SEEK_INIT_FORWARD_OP,
  SEEK_INIT_BACKWARD_OP,
  SEEK_SWITCH_FORWARD_OP,
  SEEK_SWITCH_BACKWARD_OP
};
...
class SpdbVectorContainer {
 ...
 private:
  port::RWMutex spdb_vectors_rwlock_;
...
class SpdbVectorIterator : public MemTableRep::Iterator {
 public:
  ...
  void InternalSeek(const Slice* seek_key, SeekOption seek_op) {
  ...
  void ReverseDirection(bool up_iter_direction) {
    ...
    InternalSeek(&seek_key, (up_iter_direction) ? SEEK_SWITCH_FORWARD_OP
                                                : SEEK_SWITCH_BACKWARD_OP);
  }
  ...
  void Seek(const Slice& internal_key,
    ...
    Reset(true);
    InternalSeek(&internal_key, SEEK_INIT_FORWARD_OP);
  }
  ...
  void SeekForPrev(const Slice& internal_key,
                   const char* /* memtable_key */) override {
    Reset(false);
    InternalSeek(&internal_key, SEEK_INIT_BACKWARD_OP);
  }
  ...
  void SeekToFirst() override {
    Reset(true);
    InternalSeek(nullptr, SEEK_INIT_FORWARD_OP);
  }
  ...
  void SeekToLast() override {
    Reset(false);
    InternalSeek(nullptr, SEEK_INIT_BACKWARD_OP);
  }
